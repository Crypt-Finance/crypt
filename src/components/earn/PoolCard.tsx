...
export default function PoolCard({ stakingInfo }: { stakingInfo: StakingInfo }) {
	...
	if (avaxPool) {
		...
		token = token0.equals(RIP[token0.chainId]) ? token1 : token0
	}
		...
		<Wrapper showBackground={isStaking} bgColor={backgroundColor}>
			...
			<StatContainer>
				...
				<RowBetween>
					...
					<TYPE.white>{`${weeklyRewardAmount.toFixed(0, { groupSeparator: ',' })} RIP / week`}</TYPE.white>
				</RowBetween>
				<RowBetween>
					<TYPE.white> Current reward </TYPE.white>
					<TYPE.white>{`${weeklyRewardPerAvax.toFixed(4, {groupSeparator: ','}) ?? '-'} RIP / Week per AVAX`}</TYPE.white>
				</RowBetween>
			</StatContainer>
					...
					<BottomSection showBackground={true}>
						...
						<TYPE.black style={{ textAlign: 'right' }} color={'white'} fontWeight={500}>
							...
							{`${stakingInfo.rewardRate
								?.multiply(`${60 * 60 * 24 * 7}`)
								?.toSignificant(4, { groupSeparator: ',' })} RIP / week`}
						</TYPE.black>
					</BottomSection>
				</>
			)}
